...
Currently, the most popular restraint scheme is a method by Boresch et al \cite{boresch2003absolute} that utilizes a set of orthogonal positional, angular, and dihedral restraints.  The advantage of this scheme is that it is an exact analytical expression for the free energy of restraint for a ligand in solution.    There are several disadvantages to this scheme, however.  One disadvantage is the practical difficulty of implementing three different kinds of restraints, and judiciously choosing the optimal degrees of freedom in which to impose them.  This has recently been emphasized by Proccaci et al. \cite{procacci2021ns} who show that large biases in free energies estimated using a non-equilibrium work (NEW) method can be introduced when the Boresch-restrained ligand pose does not match well with the actual pose, which is often unknown ahead of time.

In this work, we propose a simpler alternative to Boresch-style orientation restraints that uses three harmonic position restraints.  This scheme is easier to implement in practice, and is accurate when sufficiently rigid molecular substructures are restrained.  We derive an analytical expression for the free energy of restraint, and test the accuracy of the expression using a series of model compounds with various combinrations of rigid aromatic rings and flexible aliphatic chains.   These "Hurley-style" restraints may be useful in many applications due to their simplicity.
